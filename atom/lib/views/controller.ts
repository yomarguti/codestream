	...
	constructor(private session: WorkspaceSession, private viewState = initialViewState) {
		...
		containers.map(container => {
			this.subscriptions.add(
				...
				Container.session.agent.onDidChangeVersionCompatibility(
					(e: DidChangeVersionCompatibilityNotification) => {
						this.session.versionCompatibility = e.compatibility;
						if (this.mainView) {
							this.mainView.changeVersionCompatibility(e);
						}
					}
				),
				Container.session.agent.onDidChangeApiVersionCompatibility(async e => {
