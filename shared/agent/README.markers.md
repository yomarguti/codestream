...
is 1,1. These are referred to as ***locations***. The API server encodes locations as tuples in the
format ```[lineStart, colStart, lineEnd, colEnd]```.

LSP in the other hand uses 0-based coordinates, referred to as ***ranges***. Conversion between locations and
...
| ```type``` | Marker type |
| ```color``` | Marker color |
***Other fields***: ```_id```, ```deactivated```, ```createdAt```, ```modifiedAt```, 
...
***Other fields***: ```file```, ```location```, ```commitHash```, ```repoId```, ```repo```, ```streamId```
...
a [```MissingMarkerReason```](src/shared/agent.protocol.markers.ts).
...
calculating this location is referred to as ***backtracking*** and is implemented by
...
The hunks are converted to ***Changesets***, where each ***Change*** is a pair of connected additions and deletions with
