...
export class PubnubHistory {
	...
	private _allMessages: Array<any> = [];
	...
	private async fetchByChannelSlice (output: PubnubHistoryOutput, channels: Array<string>): Promise<boolean> {
	...
	private processMessages (output: PubnubHistoryOutput): Array<object> {
	...
	private async retrieveBatchHistory(channels: Array<string>, timetoken: string, depth: number = 0) {
		if (depth === 100) {
			throw "RESET";
		}
...
*/
		...
		const channelsWithMoreMessages: Array<string> = [];
		...
		for (let channel in response) {
			...
			for (let message of response.channels[channel]) {
				const timetoken = parseInt(message.timetoken, 10);
				if (
					!latestTimetokenPerChannel[channel] || 
					timetoken > latestTimetokenPerChannel[channel]
				) {
		...
		await Promise.all(channelsWithMoreMessages.map(async channel => {
