...
import { MessageType, RTMessage, StreamsRTMessage } from "../apiProvider";
...
export class SlackEvents {
	...
	private async onSlackConnectionChanged(e: any) {
		Logger.logWithDebugParams(`SlackEventReceiver.onSlackConnectionChanged`, e);
	}
	...
	private async onSlackChannelChanged(
		...
		try {
			Logger.logWithDebugParams(
				`SlackEventReceiver.onSlackChannelChanged(${type}${subtype ? `:${subtype}` : ""})`,
				e
			);
				...
				case SlackRtmEventTypes.GroupArchived: {
					const message = {
						...
						data: [
							{
								id: e.channel,
								$set: { modifiedAt: e.event.ts, isArchived: true }
							} as unknown
						]
					} as StreamsRTMessage;
				...
				case SlackRtmEventTypes.GroupJoined: {
					const message = {
						...
						data: [
							fromSlackChannelOrDirect(
								e.channel,
								await this._api.ensureUsersById(),
								this._api.userId,
								this._api.teamId
							)
						]
	...
	private async onSlackMessageChanged(
		...
		try {
			Logger.logWithDebugParams(
				`SlackEventReceiver.onSlackMessageChanged(${type}${subtype ? `:${subtype}` : ""})`,
				e
			);
				...
				case SlackRtmEventTypes.Message:
					switch (subtype) {
						case undefined: {
							if (e.user !== this._api.userId) {
								...
								try {
									...
									Logger.error(
										ex,
										`SlackEventReceiver.onSlackMessageChanged(${type}${
											subtype ? `:${subtype}` : ""
										})`
									);
								}
						...
						case SlackRtmMessageEventSubTypes.Deleted: {
							const usersById = await this._api.ensureUsersById();
						...
						default: {
							// Don't trust the payload, since it might not be a full message
							const response = await this._api.getPost({ streamId: e.channel, postId: e.ts });
							this._onDidReceiveMessage.fire({
								type: MessageType.Posts,
								data: [response.post]
							});
