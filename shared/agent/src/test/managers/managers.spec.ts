...
import { EntityManager, Id, IndexedField } from "../../managers/managers";
import { Cache } from "../../managers/cache";
import { IndexType } from "../../managers";
import { SequentialSlice } from "../../managers/sequentialSlice";
...
interface TestEntity extends CSEntity {}
...
class TestEntityManager extends EntityManager<TestEntity> {
	constructor(public readonly fetchEntity?: TestEntity) {
		super();
	}

	protected async fetch(id: Id): Promise<TestEntity> {
		return this.fetchEntity!;
	}

	protected getIndexedFields(): IndexedField<TestEntity>[] {
		return [];
	}
}
...
class PersonManager extends EntityManager<TestPerson> {
	...
	protected getIndexedFields(): IndexedField<TestPerson>[] {
		return [
			{
				field: "ssn",
...
class PostManager extends EntityManager<TestPost> {
	...
	protected getIndexedFields(): IndexedField<TestPost>[] {
		return [
			{
				field: "streamId",
