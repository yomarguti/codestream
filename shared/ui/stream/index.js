...
export class SimpleStream extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    const switchingFileStreams =
      nextProps.fileStreamId !== this.props.fileStreamId;
    const switchingPostStreams =
      nextProps.postStreamId !== this.props.postStreamId;

    if (
      nextProps.fileStreamId &&
      switchingFileStreams &&
    ...
    if (!nextProps.hasFocus && this.props.hasFocus) {
      ...
      if (this.props.currentUser && this.props.currentUser.lastReads) {
        this.postWithNewMessageIndicator = this.props.currentUser.lastReads[
          nextProps.postStreamId
        ];
      }
    }
    if (this.props.currentUser && this.props.currentUser.lastReads) {
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (this.props.posts.length !== prevProps.posts.length) {
      ...
      if (lastPost) {
        ...
        const mine =
          this.props.currentUser.username === lastPost.author.username;
  ...
  renderIntro = () => {
    return [
          ...
          <li>
            ...
            >
              {() => (
                <React.Fragment>
                  Select{" "}
                  <a onClick={this.props.goToInvitePage}>Codestream: Invite</a>{" "}
      ...
      <label key="learn-more">
        Learn more at{" "}
        <a onClick={this.handleClickHelpLink}>help.codestream.com</a>
      </label>
    ];
  };
          ...
          >
            ...
            {posts.map(post => {
              ...
              const newMessageIndicator =
                post.seqNum &&
                post.seqNum === Number(this.postWithNewMessageIndicator);
              ...
              const returnValue = (
                <div key={post.id}>
                  ...
                  <Post
                    ...
                    unread={unread}
                    editing={
                      !this.state.threadActive &&
                      post.id === this.state.editingPostId
                    }
                  />
                </div>
              );
          ...
          >
            {threadPost && (
              <Post
                ...
                editing={
                  this.state.threadActive &&
                  threadPost.id === this.state.editingPostId
                }
              />
            )}
  ...
  handleScroll(_event) {
    ...
    if (this.state.unreadsAbove != unreadsAbove)
      this.setState({ unreadsAbove: unreadsAbove });
    if (this.state.unreadsBelow != unreadsBelow)
      this.setState({ unreadsBelow: unreadsBelow });
  }
  ...
  replacePostText = (postId, newText) => {
    ...
    const mentionUserIds = this.findMentionedUserIds(
      replaceText,
...
const mapStateToProps = ({
  ...
  const teamMembers = teams[context.currentTeamId].memberIds
    .map(id => users[id])
    .filter(Boolean);
  ...
  const isOffline =
    ...
    messaging.failedSubscriptions.length > 0 ||
    messaging.timedOut;
  ...
  return {
    ...
    posts: streamPosts.map(post => {
      ...
      if (!user) {
        console.warn(
          `Redux store doesn't have a user with id ${
            post.creatorId
          } for post with id ${post.id}`
        );
        user = {
...
export default connect(mapStateToProps, {
  ...actions,
  goToInvitePage: () => {} // FIXME: routingActions.goToInvitePage
})(SimpleStream);
