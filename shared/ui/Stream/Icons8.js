var data = require("./icons8-data.json");
...
Object.keys(data).forEach(function(key) {
		...
		if (options) {
			...
			if (options["width"] || options["height"]) {
				attrObj["width"] = options["width"]
					? options["width"]
					: (parseInt(options["height"]) * data[key].options["width"]) /
					  data[key].options["height"];
	...
	data[key].options.viewBox =
		data[key].viewBox || "0 0 " + data[key].width + " " + data[key].height;
