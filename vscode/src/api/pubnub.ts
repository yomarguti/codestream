        ...
        try {
            if ((status as any).error) {
                ...
                if (errorStatus.category === Pubnub.CATEGORIES.PNAccessDeniedCategory) {
                    const response = JSON.parse(errorStatus.errorData.response.text) as PubNubAccessDeniedErrorResponse;
            ...
            try {
                ...
                switch (key as MessageType) {
                    case 'posts':
                        ...
                        this._onDidReceiveMessage.fire({ type: MessageType.Posts, posts: CodeStreamApi.normalizeResponse(entities) as CSPost[] });
                        break;
                    case 'repos':
                        entities = this.stripDirectives(key, entities);
                        if (!entities || !entities.length) continue;

                        this._onDidReceiveMessage.fire({ type: MessageType.Repositories, repos: CodeStreamApi.normalizeResponse(entities) as CSRepository[] });
                        break;
                    case 'streams':
                        entities = this.stripDirectives(key, entities);
                        if (!entities || !entities.length) continue;
