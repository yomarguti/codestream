...
export class StreamVisibilityManager {
	...
	constructor(public readonly session: CodeStreamSession, public userId: string) {
		this._hiddenStreams = new Set(
			Container.context.globalState.get<string[]>(`user:${session.userId}:streams:hidden`) || []
		);
	}
	...
	private save() {
		return Container.context.globalState.update(`user:${this.userId}:streams:hidden`, [
			...this._hiddenStreams.values()
		]);
	}
}
