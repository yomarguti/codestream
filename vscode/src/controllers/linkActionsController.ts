...
export class LinkActionsController extends Disposable {
    ...
    toLinkAction<T>(service: string, action: string, context: T, transform: LinkActionTextTransformation) {
        return `codestream://${service}/${action}?d=${encodeURIComponent(JSON.stringify(context))}&tt=${encodeURIComponent(JSON.stringify(transform))}`;
    }
