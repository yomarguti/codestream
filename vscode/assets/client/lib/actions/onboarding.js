...
export const authenticate = ({ email, password }) => (dispatch, getState, { api, http }) => {
	dispatch(requestStarted());
	return api
		...
		.then(async ({ accessToken, user, teams, repos, pubnubKey }) => {
			...
			const teamIdsForUser = user.teamIds || teams.map(team => team.id);
			...
			if (!teamIdForRepo && teams.length === 0) dispatch({ type: "NEW_USER_LOGGED_INTO_NEW_REPO" });
			else if (!teamIdForRepo && teams.length > 0) {
				await dispatch(fetchTeamMembers(teamIdsForUser));
				dispatch({ type: "EXISTING_USER_LOGGED_INTO_NEW_REPO" });
			} else if (teamIdsForUser.includes(teamIdForRepo)) {
				dispatch(fetchCompanies(teams.map(t => t.companyId)));
				await dispatch(fetchTeamMembers(teamIdsForUser));
