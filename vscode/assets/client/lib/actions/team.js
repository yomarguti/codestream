...
export const saveTeams = teams => (dispatch, getState) => {
	// return upsert(db, "teams", attributes).then(teams =>
	dispatch({ type: "ADD_TEAMS", payload: teams });
	// );
};

export const fetchTeamMembers = teamId => (dispatch, getState, { api }) => {
	if (Array.isArray(teamId)) return Promise.all(teamId.map(id => dispatch(fetchTeamMembers(id))));
	return api.fetchTeamMembers(teamId).then(({ users }) => dispatch(saveUsers(users)));
};
