...
export class SimpleUMIs extends Component {
	...
	componentWillUnmount() {
		...
		const newCount = platform.getAppBadgeCount() - this.totalCount;
		platform.setAppBadgeCount(newCount < 0 ? 0 : newCount);
	}
	...
	render() {
		...
		this.context.platform.setAppBadgeCount(Math.floor(totalUMICount)); // TODO: This needs to be smarter and it should add to the current badge count rather than replace it
		Object.keys(umis.unread).map(key => {
			let path = streamMap[key] || "";
			this.treatPath(path);
		});
